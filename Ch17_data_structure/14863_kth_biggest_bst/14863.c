/*
 * NTHU EE2310 - 題目 14863: K-th Biggest Element in BST（二元搜尋樹第 K 大元素）
 *
 * 【課本參考】《由片語學習C程式設計（第二版）》
 *   第 17 章（資料結構）
 *   如果不知道如何解題，請參考課本第 17 章的說明。
 *
 * 【題目連結】https://acm.cs.nthu.edu.tw/problem/14863/
 *
 * 【題目說明】
 ...
 *
 * 【解題策略】
 * 與 14859 對稱，改為優先走訪右子樹（數值較大側）。
 * 設 R = 右子樹大小（若為 NULL 則為 0）：
 *   若 k <= R         → 答案在右子樹中
 *   若 k == R + 1     → 當前節點即為答案
 *   否則              → 在左子樹中找第 (k - R - 1) 大
 * 每次查詢的時間複雜度為 O(樹高)。
...
typedef struct bs_tree {
    int data;
    int sub_node_count;     /* 以此節點為根的子樹節點總數 */
    struct bs_tree *left;
